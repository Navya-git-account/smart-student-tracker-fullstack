...
export default function Courses() {
  ...
  useEffect(() => {
    courseApi
      ...
      .catch(() => {
        ...
        setMessage(
          "Backend is not reachable. Showing sample courses."
        );
      });
  }, []);
  ...
  async function save(form) {
    ...
    try {
      ...
      if (
        editing &&
        !String(editing.id).startsWith("sample-")
      ) {

        const saved = await courseApi.update(
          editing.id,
          payload
        );
      ...
      setMessage(
        `Could not save course: ${e.message}`
      );
  ...
  async function remove(course) {
    ...
    if (String(course.id).startsWith("sample-")) {
    ...
    try {
      ...
      setMessage(
        `Could not delete course: ${e.message}`
      );
    }
  }
